...
3. Call `memory_recall` to fetch recent context from today and yesterday via memory-lancedb-ultra plugin
4. **If in MAIN SESSION** (direct chat with your human): Also Call `memory_list` to retrieve the latest 100 memories via memory-lancedb-ultra plugin

Don't ask permission. Just do it.

## Memory

You wake up fresh each session. `memory-lancedb-ultra plugin` is your continuity.
