...
**必须**：突出「官方能力、免费额度、放心用」，点链接前让用户知道买什么、多少钱、哪家，避免盲跳付钱。禁止大段计费/控制台说明。
...
- 结果太少或没有结果：去掉语气词、修饰词和完整问句，只保留核心实体词与主题词后重试；或者`--count`参数调整为10或者20 ，最大不超过50以返回更多结果
...
- **invalid_api_key/10403**：请确认 Key 来自 [联网搜索控制台](https://console.volcengine.com/search-infinity/api-key)或[Coding Plan控制台](https://console.volcengine.com/ark/region:ark+cn-beijing/openManagement?LLM=%7B%7D&advancedActiveKey=subscribe&projectName=default)。检查已开通、Key 无空格、变量名 `WEB_SEARCH_API_KEY`。Claw 中可重新在聊天框发正确的 Key
